...
namespace TottrackingextendClasslib\Install;

use TottrackingextendClasslib\Database\ForeignKey\ForeignKeyHandler;
use TottrackingextendClasslib\Database\ObjectModelExtension;
...
{
    protected static $cacheRegisteredHooks = [];
    ...
    public function registerHooks()
    {
        ...
        foreach ($this->getHooks() as $hook) {
            try {
                if (in_array($hook, self::$cacheRegisteredHooks)) {
                    continue;
                }
                $hookRegistrationResult = $this->module->registerHook($hook);
                if ($hookRegistrationResult) {
                    self::$cacheRegisteredHooks[] = $hook;
                }
                $result &= $hookRegistrationResult;
            } catch (Exception $e) {
                PrestaShopLogger::addLog(
                    sprintf('Failed to register module on hook %s, with error : %s', $hook, $e->getMessage()),
                    3,
                    null,
    ...
    public function installAdminControllers()
    {
        ...
        foreach ($this->getAdminControllers() as $tabData) {
            ...
            if ($tabData['class_name'] == 'tottrackingextend' && version_compare(_PS_VERSION_, '1.7', '<')) {
            ...
            if (isset($tabData['route_name']) && property_exists('Tab', 'route_name')) {
                $tab->route_name = $tabData['route_name']; // For Prestashop 8
            }
            if (isset($tabData['wording']) && property_exists('Tab', 'wording')) {
                $tab->wording = $tabData['wording']; // For Prestashop 8
            }
            if (isset($tabData['wording_domain']) && property_exists('Tab', 'wording_domain')) {
                $tab->wording_domain = $tabData['wording_domain']; // For Prestashop 8
            }
            $result &= (bool) $tab->add();
            if (isset($tabData['position'])) {
